-- race_features_poc view definition with extended POC features
...
WITH base AS (
  SELECT
    ...
    -- Career totals
    (SELECT COUNT(*) FROM `horse-predictor-v2.horse_data_v2.RACE_RECORDS` rr_tot WHERE rr_tot.horse_id = rr.horse_id) AS career_race_count,
    ...
    -- Pedigree influence
    h.father_id,
    (SELECT COUNT(*) FROM `horse-predictor-v2.horse_data_v2.RACE_RECORDS` rr_scount
       JOIN `horse-predictor-v2.horse_data_v2.HORSES` h_s ON rr_scount.horse_id = h_s.horse_id
      WHERE h_s.father_id = h.father_id
    )                                     AS sire_win_count,
    (SELECT SAFE_DIVIDE(COUNTIF(rr_s.finish_place=1), COUNT(*))
       FROM `horse-predictor-v2.horse_data_v2.RACE_RECORDS` rr_s
       JOIN `horse-predictor-v2.horse_data_v2.HORSES` h_s2 ON rr_s.horse_id = h_s2.horse_id
      WHERE h_s2.father_id = h.father_id
    )                                     AS sire_win_pct,
  ...
  JOIN `horse-predictor-v2.horse_data_v2.HORSE_CAREERS` hc ON rr.horse_id = hc.horse_id AND EXTRACT(YEAR FROM r.race_date) = hc.race_year
